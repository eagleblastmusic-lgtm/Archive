...
def _ensure_catalog_indexing(service) -> None:
    """Schedule cache import and durable indexing without delaying an API response."""
    ...
    def worker():
        try:
            ...
            if _published_catalog_revision(service) is None:
                service.build_revision_background(_catalog_fetchers(), force=False)
        except Exception as exc:
            with _catalog_bootstrap_lock:
            ...
            with _catalog_bootstrap_lock:
                ...
                if service_key not in _catalog_bootstrap_retry_at:
...
def progressive_feed(
    ...
    if active_rev is not None or available_rev is not None:
        ...
        return catalog_service.query_page(
            page=page,
            page_size=HOME_PAGE_SIZE,
            source=source,
            author_filter=author_filter,
