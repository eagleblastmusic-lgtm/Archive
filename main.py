...
def get_thumbnail_proxy(url: str = Query(...)):
    """Błyskawiczne serwowanie miniatur z pamięci RAM (0.2 ms) lub dysku z blokadą singleflight."""
    ...
    # 1. Błyskawiczny odczyt z RAM (szybka ścieżka bez blokowania zamka pobierania)
    ...
    # 2. Blokada singleflight na poziomie danego URL – współbieżne zapytania o tę samą miniaturę czekają na 1 pobranie
    with _thumb_fetch_lock(url_hash):
        # Ponowne sprawdzenie RAM po uzyskaniu blokady (na wypadek gdy inny wątek właśnie zakończył pobieranie)
        with MEMORY_CACHE_LOCK:
            if url_hash in MEMORY_CACHE:
                content, content_type = MEMORY_CACHE[url_hash]
                return Response(content=content, media_type=content_type, headers={
        ...
        # 3. Pobierz z sieci (tylko 1 wątek na raz dla tego samego URL)
        try:
            ...
            if res.status_code == 200:
                ...
                try:
                    ...
                    # Limit cache dyskowego: sprawdzany co 200 nowych zapisów i nie częściej niż raz na 5 minut
                    global _thumb_disk_writes, _last_thumb_trim_time
                    _thumb_disk_writes += 1
                    now = time.time()
                    if _thumb_disk_writes >= 200 and (now - _last_thumb_trim_time) > 300:
                        _thumb_disk_writes = 0
                        _last_thumb_trim_time = now
...
async def toggle_favorite(video: dict = Body(...)):
...
async def record_history(video: dict = Body(...)):
...
def get_catalog_stats(source: str = "all", author_filter: str = "all") -> dict:
    """Dynamicznie przelicza całkowitą liczbę filmów i stron w katalogu po usunięciu/zablokowaniu autorów."""
    blocked_stats = storage.get_blocked_stats()
    blocked_vids = blocked_stats.get("blocked_videos_total", 0)

    # Baza katalogu aplikacji przed usunięciem autorów (250 stron × 280 filmów = 70 000)
    base_catalog = 70000
    if blocked_vids >= base_catalog:
        base_catalog = blocked_vids + (HOME_PAGE_SIZE * 20)

    if source == "only-archivebate":
        base_videos = int(base_catalog * 0.52)
        deducted = int(blocked_vids * 0.35)
    elif source == "only-camwhores":
        base_videos = int(base_catalog * 0.48)
        deducted = int(blocked_vids * 0.65)
    else:
        base_videos = base_catalog
        deducted = blocked_vids

    final_total = max(base_videos - deducted, HOME_PAGE_SIZE)
    final_last_page = max(1, math.ceil(final_total / HOME_PAGE_SIZE))

    # Twardy limit fizyczny: Archivebate ma dokładnie 1000 podstron (po 36 filmów).
    # Przy partii 20 podstron na 1 stronę aplikacji, maksymalna liczba stron to 50.
    if source == "only-archivebate":
        final_last_page = min(final_last_page, 50)
        final_total = min(final_total, 50 * HOME_PAGE_SIZE)
...
def get_videos(
    ...
    if not cached and source == "all" and author_filter == "all" and str(group_authors).lower() not in ("1", "true"):
    ...
    enriched = _enrich_videos(raw_videos, author_filter=author_filter, source=source, group_authors=group_authors)

    # Zabezpieczenie: jeśli po usunięciu zablokowanych profili lub grupowaniu liczba spadnie poniżej 280, dociągamy brakujące wideo z kolejnych stron
    current_extra_p = page + 1
    max_extra_p = page + 4 if str(group_authors).lower() in ("1", "true") else page + 1
    while len(enriched) < HOME_PAGE_SIZE and author_filter != "only_fav" and current_extra_p <= max_extra_p:
        extra_videos = scraper.get_home_videos(
            page=current_extra_p,
            source=source,
            author_filter=author_filter,
            target_count=HOME_PAGE_SIZE
        )
        ...
        raw_videos = combined
        enriched = _enrich_videos(combined, author_filter=author_filter, source=source, group_authors=group_authors)
        try:
...
def _normalize_video_details(video_id: str, details: dict) -> dict:
    ...
    # Odzyskiwanie nazwy autora gdy scraper zwrócił 'Model' lub pustą wartość
    ...
    if not curr_u or curr_u.lower() == "model":
        ...
        recovered = None
        if title:
            first_token = re.split(r'[-_\s]', title)[0].strip('-_.,')
            if re.match(r'^[a-zA-Z0-9_]{3,25}$', first_token) and first_token.lower() not in ["the", "hot", "cum", "cam", "sex", "live", "show", "video", "best", "new", "real", "watch", "free"]:
                recovered = first_token
        if not recovered and url:
            m = re.search(r'/videos/\d+/([a-zA-Z0-9_\-]+)', url)
            if m:
                p0 = re.split(r'[-_]', m.group(1))[0].strip('-_.,')
                if re.match(r'^[a-zA-Z0-9_]{3,25}$', p0) and p0.lower() not in ["the", "hot", "cum", "cam", "sex", "live", "show", "video", "best", "new", "real", "watch", "free"]:
                    recovered = p0
        if not recovered and keywords:
            for kw in keywords:
                k_clean = kw.strip().lower()
                if 3 <= len(k_clean) <= 25 and k_clean not in ["chaturbate", "stripchat", "onlyfans", "camwhores", "feet", "anal", "blowjob", "couple", "milf", "teen", "trans", "footjob", "toes", "soles", "dildo"]:
                    if (title and k_clean in title.lower()) or (url and k_clean in url.lower()):
                        recovered = kw.strip()
                        break
        if recovered:
...
def get_video_details(id: str = Query(...), force_refresh: bool = Query(False)):
    ...
    if isinstance(cached, dict) and cached and cached.get("direct_url") and not force_refresh and age <= DETAILS_CACHE_STALE_SECONDS:
        details = cached
        ...
        if hasattr(scraper, "_details_cache"):
            scraper._details_cache.pop(clean_id, None)
        clean_cw = clean_id.replace("cw_", "")
        if hasattr(camwhores_scraper, "_details_cache"):
            camwhores_scraper._details_cache.pop(clean_cw, None)
            camwhores_scraper._details_cache.pop(clean_id, None)
...
_stream_adapter = HTTPAdapter(pool_connections=100, pool_maxsize=100, max_retries=Retry(total=2, backoff_factor=0.1))
...
def stream_video_proxy(url: str = Query(None), id: str = Query(None), embed: str = Query(None), request: Request = None):
    ...
    if not url and clean_id:
        ...
        if isinstance(cached_details, dict) and cached_details.get("direct_url") and cache_age_seconds(cached_mtime) <= DETAILS_CACHE_STALE_SECONDS:
    ...
    # Dedykowana sesja stream_session z pulą 100 połączeń HTTP i keep-alive
    active_session = stream_session

    try:
        req = _validated_session_get(active_session, url, headers=headers, stream=True, timeout=12)

        # Jeśli URL wygasł lub serwer zwrócił błąd autoryzacji / wygaśnięcia linku
        if req.status_code in (401, 403, 404, 410, 502) and clean_id:
            if hasattr(scraper, "_details_cache"):
                scraper._details_cache.pop(clean_id, None)
            clean_cw = clean_id.replace("cw_", "")
            if hasattr(camwhores_scraper, "_details_cache"):
            ...
            try:
                ...
                pass
            try:
                os.remove(_details_cache_path(clean_cw))
            except OSError:
                pass
        ...
        def iterfile():
            try:
                first = True
                for chunk in req.iter_content(chunk_size=1024 * 64 if first else 1024 * 256):
                    if chunk:
                        yield chunk
                    if first:
                        first = False
...
async def unblock_model_endpoint(username: str):
