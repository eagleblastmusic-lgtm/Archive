...
def _extract_segment_frames(
    ...
    """Jedno wywołanie FFmpeg dekodujące przedział czasowy segmentu na klatki 1 fps."""
    cmd = [
        ffmpeg,
        "-hide_banner", "-loglevel", "error", "-nostdin",
        "-ss", f"{start_time:.3f}",
        ...
        "-i", source_url,
        "-vf", f"fps={SEGMENT_FPS},scale={FRAME_WIDTH}:{FRAME_HEIGHT}:force_original_aspect_ratio=increase,crop={FRAME_WIDTH}:{FRAME_HEIGHT}",
        "-an", "-sn", "-dn",
        "-threads", "1",
        "-q:v", "7",
    ...
    try:
        ...
        if frames and res.returncode == 0:
            return frames
    except Exception:
        pass

    # Fallback jeśli seek przed -i nie zadziałał na nietypowym strumieniu
    try:
        cmd_fallback = [
            ffmpeg,
            "-hide_banner", "-loglevel", "error", "-nostdin",
        ...
        subprocess.run(cmd_fallback, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, timeout=timeout)
    except Exception:
        pass
