...
def start(video_id: str, duration: float, source_url: str, force: bool = False) -> dict:
        ...
        if not current or current.get("upgrade_status") not in ("building", "ready"):
            def upgrade_worker():
                ...
                try:
                    ...
                    with _state_lock:
                        _states[k] = {"status": "ready", **full_result, "upgrade_status": "ready"}
                except Exception as exc:
                    with _state_lock:
                        _states[k] = {"status": "ready", **quick, "upgrade_status": "error", "upgrade_error": str(exc)}
    ...
    with _state_lock:
        _states[k] = {"status": "building", "stage": "quick", "started_at": int(time.time())}

    def worker():
        try:
            quick_result = _build_variant(video_id, duration, source_url, "quick")
            with _state_lock:
                _states[k] = {"status": "ready", **quick_result, "upgrade_status": "building"}
            try:
                full_result = _build_variant(video_id, duration, source_url, "full")
                with _state_lock:
                    _states[k] = {"status": "ready", **full_result, "upgrade_status": "ready"}
                ...
                with _state_lock:
                    _states[k] = {"status": "ready", **quick_result, "upgrade_status": "error", "upgrade_error": str(full_exc)}
        except Exception as exc:
            with _state_lock:
                _states[k] = {"status": "error", "error": str(exc), "finished_at": int(time.time())}
